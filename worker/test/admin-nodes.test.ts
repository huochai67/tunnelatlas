...
describe("admin node management", () => {
  ...
  it("updates subscription visibility for an existing tunnel", async () => {
    const { env, calls } = testEnv([
      { id: "inbound-1", node_id: "node_one", name: "public", subscription_enabled: 0 },
      { id: "inbound-1", node_id: "node_one", name: "public", subscription_enabled: 1 },
    ]);
    const hideResponse = await worker.fetch(adminRequest("/v1/admin/nodes/node_one/tunnels/inbound-1", "PATCH", { subscriptionEnabled: false }), env);
    expect(hideResponse.status).toBe(200);
    expect(await hideResponse.json()).toEqual({
      tunnel: { id: "inbound-1", nodeId: "node_one", name: "public", subscriptionEnabled: false },
    });

    const restoreResponse = await worker.fetch(adminRequest("/v1/admin/nodes/node_one/tunnels/inbound-1", "PATCH", { subscriptionEnabled: true }), env);
  ...
  it("rejects non-boolean subscriptionEnabled payloads", async () => {
    ...
    for (const value of invalidValues) {
      const response = await worker.fetch(adminRequest("/v1/admin/nodes/node_one/tunnels/inbound-1", "PATCH", { subscriptionEnabled: value }), env);
      expect(response.status).toBe(400);
      const body = await response.json() as Record<string, any>;
      expect(body.title).toBe("Invalid subscriptionEnabled");
    }
    const nullBodyResponse = await worker.fetch(adminRequest("/v1/admin/nodes/node_one/tunnels/inbound-1", "PATCH", null), env);
    ...
    const arrayBodyResponse = await worker.fetch(adminRequest("/v1/admin/nodes/node_one/tunnels/inbound-1", "PATCH", []), env);
  ...
  it("returns 404 when updating a missing tunnel", async () => {
  ...
  it("requires ADMIN_TOKEN before updating tunnel subscription visibility", async () => {
    const { env, calls } = testEnv();
    const response = await worker.fetch(new Request("https://atlas.example/v1/admin/nodes/node_one/tunnels/inbound-1", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
