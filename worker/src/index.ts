...
async function updateTunnelSubscription(
  ...
  requireToken(request, env.ADMIN_TOKEN);
  const nodeId = resourceId(encodedNodeId);
  const tunnelId = resourceId(encodedTunnelId);
  ...
  const updated = await env.DB.prepare(
    "UPDATE tunnels SET subscription_enabled = ? WHERE node_id = ? AND id = ? RETURNING id, node_id, name, subscription_enabled",
  ).bind(body.subscriptionEnabled ? 1 : 0, nodeId, tunnelId).first<{
    id: string;
    node_id: string;
    name: string;
    subscription_enabled: number;
  }>();
  if (!updated) throw new HttpError(404, "Tunnel not found");
  return json({
    tunnel: {
      id: updated.id,
...
function tunnelQuery(filter: boolean, onlineOnly: boolean, subscriptionOnly = false): string {
  return `SELECT t.id, t.node_id, t.name, t.kind, t.endpoint, t.protocol, t.status,
    t.metadata_json, t.authentication_ciphertext, t.last_seen_at, t.subscription_enabled, n.name AS node_name,
    ...
    WHERE n.public_key IS NOT NULL ${onlineOnly ? "AND n.last_seen_at >= ?" : ""} ${filter ? "AND t.node_id = ?" : ""} ${subscriptionOnly ? "AND t.subscription_enabled = 1" : ""}
